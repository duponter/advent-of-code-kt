...
class Day03 {
    ...
    fun part2(input: Text): Int =
            input.chars()
                    ...
                    .reduce({ list1, list2 -> list1.plus(list2) }) //  Santa Points + Robo Points --> List<Point>
                    .toSet()    // Keep unique only
                    .size

    private fun deliver(input: List<Char>): List<Point> = input
            .foldRight(listOf(Point(0, 0)), { c, list -> list.plus(list.last().move(c)) })
}
